...
namespace CoreApp.Application.Implementation
{
    public class BillService : IBillService
    {
        ...
        public void Update(BillViewModel billVm)
        {
            ...
            var listExisted = existedDetails.ToList();
            var listUpdated = updatedDetails.ToList();

            List<BillDetail> listDeleted = new List<BillDetail>(listExisted);
            foreach (var updated in listUpdated)
            {
                foreach (var deleted in listDeleted)
                {
                    if (deleted.Id == updated.Id)
                    {
                        listDeleted.Remove(deleted);
                        break;
                    }
                }
            ...
            _orderDetailRepository.RemoveMultiple(listDeleted);
